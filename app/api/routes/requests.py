...
def _raise_http_from_app_error(exc: Exception) -> None:
    ...
    raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail='Внутренняя ошибка сервера') from exc
...
async def list_requests(
    ...
    try:
        items = await request_repo.get_requests(current_user.id, filters, current_user.role)
        return RequestListOut(items=items, page=filters.page, limit=filters.limit, total='')
    except Exception as exc:
        _raise_http_from_app_error(exc)
...
async def update_request_status(
    ...
    try:
        return await request_repo.update_status(current_user, payload)
    except Exception as exc:
        _raise_http_from_app_error(exc)
...
async def delete_request(
    ...
    if current_user.role != Role.admin:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail='Удалять заявки может только админ')
